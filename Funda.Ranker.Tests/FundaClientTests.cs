...
namespace Funda.Ranker.Tests
{
    ...
    public class FundaClientTests
    {
        ...
        public async Task GetObjectsForSaleShouldReturnNumberOfItemsRequested()
        {
            ...
            var sut = new FundaClient(logger.Object, new FundaConfiguration() { MaxRetries = 10, SleepTimeAfterExceedingRequestLimit = 5000, BaseUrl = "http://partnerapi.funda.nl/feeds/Aanbod.svc/json/ac1b0b1572524640a0ecc54de453ea9f/?type={0}{1}&page={2}&pagesize={3}" });
            var result = await sut.GetObjects(ListingType.Sale, 1, 25, 1, "tuin", "amsterdam").ConfigureAwait(false);
